...
public interface IProductRepository  extends CrudRepository<Product, Long> {
    ...
    @Query("Select new com.antonio.sistema_stock.dto.dtoResponse.ProductDtoResponse(p.id,p.name, p.price, p.stock, p.image, p.description , p.enable) from Product p where p.user=?1 and p.enable=true ")
    List<ProductDtoResponse> findAll(User user);
